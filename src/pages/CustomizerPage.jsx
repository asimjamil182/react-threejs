    ...
    <>
      ...
      <main className="w-full h-[calc(100vh-6rem)] bg-gray-300 flex justify-between flex-wrap md:flex-nowrap">
        <div className="w-1/5 h-full overflow-auto bg-white">
          <Customizer direction={direction} activeColor={(color) => setColor(color)} activeDecals={(decals) => setDecals(decals)} selectedLayer={(layer) => setSelectedLayer(layer)} updateProperties={properties} />
        </div>
        ...
        <div className="w-1/5 h-full overflow-auto bg-slate-200">
          <PropertiesPanel selectedLayer={selectedLayer} PropertiesUpdate={(pro) => setProperties(pro)} />
        </div>
      </main>
